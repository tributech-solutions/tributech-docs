...
- **REST Pre-defined Payload**: Data will be provided in a pre-defined format containing a unique combination of
...
- **REST Stream**: Data will be provided with the DataStreamId of the Twin Configuration contained in the data (see
...
In contrast to a generated identifier of the REST Stream we can now define a two string's used to identify the DataStreamId for the REST Source. The pre-defined payload will be added by right clicking the REST Source and selecting **_Add Streams_**. The following dialog will appear:
...
Examples on how to submit data to the REST Source can be found in the Swagger UI under **Stream** directly, e.g. double stream:
...
- **StreamEncoding**: Sets the datatype which will be generated based on a numeric value:
...
- **Interval**: The interval in which the values are generated (in TimeSpan format, e.g. 00:00:05 for 5 seconds, see [Microsoft](https://learn.microsoft.com/en-us/dotnet/standard/base-types/standard-timespan-format-strings))
- **MinValue** (default -1000): The minimum value that can be generated (for bytearrays and string this is the length of the array/string, default 1)
- **MaxValue** (default 1000): The max value that can be generated (for bytearrays and string this is the length of the array/string)
