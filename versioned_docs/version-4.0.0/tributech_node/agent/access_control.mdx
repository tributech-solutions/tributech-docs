...
In this section we show the available management function, provided by a Tributech Node UI, to manage [Tributech Agents](../../tributech_agent/overview.md).
...
For already activated [Tributech Agents](../../tributech_agent/overview.md) we provide the following operations, via the `Actions` context menu:
...
it can be configured in the ***Agents*** Overview by selecting the entry or via the action context menu ([Agent Management](./management.mdx)).
...
To confirm the deletion the Tributech Agent Id is required (can be used from the Dialogbox or via [Get Agent Id](./management.mdx#get-the-agent-id))
