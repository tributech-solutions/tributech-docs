...
will be able to submit data is defined in [Digital Twin Configuration of an Agent](management.mdx).
...
- `EnrollmentOptions__EnrollmentCertBase64` and `EnrollmentOptions__EnrollmentKeyBase64` are base64 encoded certifcates required to authenticate with a [Tributech Node](../tributech_node/overview.md) (see [Enrollment](agent_enrollment.mdx)) 
...
To complete the configuration of a Tribtech Agent visit the [configuration section](operation.mdx#configure-agent).
