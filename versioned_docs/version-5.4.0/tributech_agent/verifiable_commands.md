...
A [command](../tributech_node/api_category/API_usage.md#commands) is trusted because it arrives through the authenticated Tributech Node API. A **Verifiable Command** adds a second, end-to-end layer of trust: the command is signed by the user and re-signed by the Tributech Node, so the receiving device can independently verify its authenticity. This guarantees that:
...
1. **User → Node:** the user signs the command body with their **private key** and sends it to the Tributech Node. The Node looks up the matching **public key** (via the `nodeKeyId`) and verifies the signature — if verification fails, the command is rejected.
2. **Node → Device:** after verifying the user, the Tributech Node signs the command with its own key and forwards it to the agent/device. The device verifies this signature against the known Tributech Node public key and **executes the command only if it is valid**.
