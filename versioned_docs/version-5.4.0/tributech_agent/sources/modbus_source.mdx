...
**WordOrder** decides in which order the registers of a value spanning several registers are combined, and is set per stream and per parameter. Take an `INT32` stream on `HoldingRegister:100`, which reads two registers:

| Register | Content on the device |
| --- | --- |
| `HoldingRegister:100` | `0x1234` |
| `HoldingRegister:101` | `0x5678` |

The same two registers then produce two different values, depending on which one is taken as the high half:

| WordOrder | Combined | Published value |
| --- | --- | --- |
| `HighWordFirst` | `0x1234` `0x5678` = `0x12345678` | `305419896` |
| `LowWordFirst` | `0x5678` `0x1234` = `0x56781234` | `1450709556` |
...
- If something else changes the address on the device, i.e. another system or a [WriteOnce Command](#writeonce-command), the Source writes the configured **Value** back on the next cycle. A `WriteOnce` onto a parameter address is therefore undone again.
