...
| **Expected Health Frequency** | [ISO 8601 duration](https://en.wikipedia.org/wiki/ISO_8601#Durations) | No | — | How often the Source reports its [health state](#health-states), i.e. `PT30S` for 30 seconds, `PT5M` for 5 minutes, `PT1H` for one hour. Left empty, the Source sends no health messages. |
...
With an **Expected Health Frequency** configured on the source twin (see [Modbus Source](#modbus-source)), the Modbus Source reports one state for the whole source to the Tributech Node in that interval (see [Health Check](../../tributech_node/agent/health_checks.mdx)). The state covers every device and every connection:
