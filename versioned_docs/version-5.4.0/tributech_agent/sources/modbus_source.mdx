...
We can now add a new Modbus Source with the three dots menu on the right side of the Device Edge entry. The Source itself needs no settings beyond its name, the connection settings live on the devices below it.
