...
- `EnrollmentOptions__EnrollmentCertBase64` and `EnrollmentOptions__EnrollmentKeyBase64` are base64 encoded certificates required to authenticate with a [Tributech Node](../tributech_node/overview.md) (see [Agent Management](../tributech_node/agent/agent_management.mdx)) 
...
/>
 ...
 If too many agents are listed we can search for the Agent with the `AGENT_ID` we defined docker compose file (value for `EdgeDeviceOptions__AgentID` in our example `d7996ca3-5937-4720-afe3-d2cf8f5ac210`). By clicking on the ***Activate*** button we grant our agent the permission to connect and be managed by this [Tributech Node](../tributech_node/overview.md). For more information on agent management in the [Tributech Node](../tributech_node/overview.md) visit [agent management](../tributech_node/agent/agent_management.mdx).
