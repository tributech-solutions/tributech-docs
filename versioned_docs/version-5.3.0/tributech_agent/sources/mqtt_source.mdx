...
- **MQTT Jsonpath Collection**: A collection of streams that share an MQTT topic and use JSONPath expressions to extract individual values and optional timestamps from JSON payloads.
...
In this section, we configure a JSONPath Collection that groups multiple streams under one shared MQTT Topic. Enter the MQTT Topic on which the JSON object is published, for example factory/power-monitor. Each stream in the collection then uses a JSONPath expression to extract an individual value and, optionally, a timestamp from the received payload.
...
The topic is not a JSONPath expression. Based on the current concept document, I would use an exact topic rather than a wildcard unless wildcard support is explicitly implemented for JSONPath Collections.
