...
import NodeRESTAddSourceLight from '../img/node-source-rest-add-light.png';
import NodeRESTAddSourceDark from '../img/node-source-rest-add-dark.png';
import NodeRESTAddStreamLight from '../img/node-source-rest-add-stream-stream-light.png';
import NodeRESTAddStreamDark from '../img/node-source-rest-add-stream-stream-dark.png';
import NodeRESTStreamConfigLight from '../img/node-source-rest-add-stream-stream-config-light.png';
import NodeRESTStreamConfigDark from '../img/node-source-rest-add-stream-stream-config-dark.png';
import NodeRESTAddPayloadLight from '../img/node-source-rest-add-stream-payload-light.png';
import NodeRESTAddPayloadDark from '../img/node-source-rest-add-stream-payload-dark.png';
import NodeRESTPayloadConfigLight from '../img/node-source-rest-add-stream-payload-config-light.png';
import NodeRESTPayloadConfigDark from '../img/node-source-rest-add-stream-payload-config-dark.png';
...
The Tributech Agent supports the integration of external data sources using [REST](https://learn.microsoft.com/en-us/azure/architecture/best-practices/api-design#what-is-rest) with the  Tributech REST Source. The REST Source is configured via the Twin Configuration and is used to send data to the Tributech Agent via [MQTT](https://mqtt.org/).
 The data can be send to the Tributech Source via [Swagger](https://swagger.io/specification/).
...
We will show in [Configuration](#configuration) how to configure the REST Source to accept data for each specific stream type and provide the examples for [Swagger](#rest-endpointswagger).
...
<ThemedImage
  alt="Add REST Source"
  sources={{ light: NodeRESTAddSourceLight, dark: NodeRESTAddSourceDark }}
/>


### REST Stream
At first we add the REST Stream to our newly created Tributech REST Source. The REST Stream will be used to send data to the Tributech Node based on an unique identifier contained in the json payload. The REST Stream can be added with the three dots menu on the right side of the REST Source, selecting ***Add Streams***. The following dialog will appear:

<ThemedImage
  alt="Add REST Stream"
  sources={{ light: NodeRESTAddStreamLight, dark: NodeRESTAddStreamDark }}
/>
...
<ThemedImage
  alt="REST Stream config"
  sources={{ light: NodeRESTStreamConfigLight, dark: NodeRESTStreamConfigDark }}
/>
...
In contrast to a generated identifier of the REST Stream we can now define a two string's used to identify the DataStreamId for the REST Source. The pre-defined payload will be added with the three dots menu on the right side of the REST Source, selecting ***Add Streams***. The following dialog will appear:

<ThemedImage
  alt="Add Pre-defined Payload Stream"
  sources={{ light: NodeRESTAddPayloadLight, dark: NodeRESTAddPayloadDark }}
/>

After adding the REST Stream we configured the Data Encoding and one or both of the identifiers (Source/Value).
...
<ThemedImage
  alt="Pre-defined Payload Stream config"
  sources={{ light: NodeRESTPayloadConfigLight, dark: NodeRESTPayloadConfigDark }}
/>
