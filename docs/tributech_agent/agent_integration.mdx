...
The following table shows an example of a test Tributech Agent with a simulated source.
Please consider that the consumption may vary dependended on used source.
...
| Endpoint URL | __name__.tributech-node.com | MQTT endpoint on server (platform) side. |
