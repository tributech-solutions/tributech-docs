...
In this section we configure an additional MessageBroker connection which is different from the `source-mqtt` from the `docker-compose.yml`.
This MessageBroker should does not make the `source-mqtt` obsolete but provides the possibility to use a different MessageBroker for the MQTT Source data ingress.
Per default the MQTT Source is configured to use the same the MessageBroker `source-mqtt` service on port 1833 like the Tributech Agent (need to match your `mosquitto-server-mqtt` service in the `docker-compose.yml`) to receive data.
This holds true if the `MQTT Host` and `MQTT Port` is set in the MQTT Source configuration.
