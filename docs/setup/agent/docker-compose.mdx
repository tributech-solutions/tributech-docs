...
* Login to the private Docker registry to allow retrieval of Tributech Edge Docker images.
    ```bash
    echo <access-token> | docker login --username <token-name> --password-stdin tributech.azurecr.io
    ```
  Get "Docker Registry Token (DSK Edge)" from DataSpace-Admin API Keys area:
  <ApiCredentials />
...
The "Promtail (Loki push API)" password can be retrieved in the DataSpace-Admin API Keys area (see previous step).
...
* Startup the Tributech Edge services (will automatically retrieve the Docker images).
    ```bash
    docker-compose up -d
    ```
* You can now continue with [**connecting to, linking & configuring**](../agent_companion.mdx#agent-companion-connect) the Tributech Edge Agent.
