...
The Tributech Agent Companion is an application for the management of the DSK Agent Edge & OEM.
It allows to link them to a DSK Node and configure them based on digital twins.
It offers a twin builder UI (which can also be used stand-alone).
It communicates with the APIs of the DSK Node and DSK Agent Edge / OEM and must be connected accordingly.
